...
// The WarningQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type WarningQueryRuleFunc func(context.Context, *ent.WarningQuery) error

// EvalQuery return f(ctx, q).
func (f WarningQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.WarningQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.WarningQuery", q)
}

// The WarningMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type WarningMutationRuleFunc func(context.Context, *ent.WarningMutation) error

// EvalMutation calls f(ctx, m).
func (f WarningMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.WarningMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.WarningMutation", m)
}
